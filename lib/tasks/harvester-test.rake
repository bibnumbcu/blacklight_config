namespace :harvester do
  ...
  desc "Harvester"
  task :harvest_repository_test , [:url, :name, :set] =>  :environment do |t, args|
   #moissonnage et génération du fichier csv pour l'import

   harvester = Harvester.new(args[:name], args[:url], args[:set])
   harvester.harvest
   commandStr = 'java -Xms1024m -Xmx1024m  -Dsolr.hosturl=http://127.0.0.1:8080/solr/blacklight  -jar /usr/applications/solrmarc/lib/SolrMarc.jar config/SolrMarc/config.properties ' + File.expand_path(File.join(Rails.root, 'tmp/moissonnage.mrc'))
